...
def chat_bot(chat):
    ...
    if '개' in chat or 'lqkf' in chat or '발' in chat or '끼' in chat or '년' in chat or '놈' in chat:
        ...
        if xnt > 2:
            ...
            str1 = input("식만 써주세요. >>> ")
        return calculator(expression=str1)
    elif '타이' in chat:
        times = int(input("얼만큼? "))
        print(times)
        time.sleep(1)
        ...
        url = 'https://classic.minecraft.net/?join=XqbiL2-611QDfn7m'
        webbrowser.open(url)
        return "    "
...
while True:
    ...
    result = chat_bot(chat=chatting)
    for eng in en:
        if eng in chatting:
            xnt += 1
    if xnt > 2:
        if 'ranslate' in chatting:
            print(result)
        else:
            result = tr.translate(result, dest='en').text
            print(result)
    else:
        print(result)
    if '그만' in chatting or '끝' in chatting or '잘가' in chatting:
        time.sleep(1)
        break
    f.write(">>> " + chatting)
    f.write("""
...
qu = ['x의 ?배에 y의 ?배를 더한것은 ?과 같고, x의 ?배에 y의 ?배를 뺀것은 ?과 같다',
